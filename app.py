...
from flask import Flask,request,render_template,redirect,url_for,flash,session,send_file # type: ignore
...
mydb=connection.MySQLConnection(user='root',password='Jabeen@2004',host='localhost',db='snmprj')
app=Flask(__name__)
...
def register():
    if request.method=='POST':
        print(request.form)
        username=request.form['username'] #anusha
        email=request.form['email'] #anusha@codegnan.com
...
def otpverify(udata):
    if request.method=='POST':
        ...
        try:
            ...
            # returns {'username':'anusha','useremail':'anusha@codegnan.com','userpassword':'123','usergender':'Female','stored_otp':'Ki8Nh6'}
...
def login():
    if not session.get('user'):
        if request.method=='POST':
            uemail=request.form['useremail'] #user email
            ...
            if count_useremail[0]==1:
                ...
                if stored_password[0]==password:
                    ...
                    flash('password was wrong pls check')
                    return redirect(url_for('login'))
...
def forgotpassword():
    if request.method=='POST':
        ...
        if count_useremail[0]==1:
            ...
            flash(f'Reset link has been sent given email {useremail}')
...
def viewnotes(nid):
    if session.get('user'):
        ...
        notesdata=cursor.fetchone() #(1,'python','dfghjk','2025-09-23','anusha@codegnan.com')
...
def updatenotes(nid):
    if session.get('user'):
        ...
        if request.method=='POST':
            ...
            mydb.commit()
            cursor.close()
            flash(f'notes updated successfully{title}')
            return redirect(url_for('viewallnotes'))
